...
- 新增“人才培养方案 / 课程计划”查询页面，展示基础必修、专业必修/选修、全校通识、实验/实践等学分要求与选课窗口，管理员可在后台调整或新建方案。
- 自定义管理后台：教学班支持上课时间内联编辑，选课记录/课程/教师/学生均配置了列表筛选与搜索，方便教务人员使用。
- 学生/教师门户：登录后看到类似管理员的仪表盘，提供课表预览、审批进度、成绩概览、重修提醒、成绩录入与审批入口。
- 管理员工作台提供课程通过率、学生 GPA 分布等成绩统计视图，方便实时监测教学质量；教师工作台新增教学班成绩分析（均分、最高/最低分、通过率）。
...
3. **导入示例数据（含管理员与演示账号）**
   - **大规模演示数据（课程/学生齐全，用于全面演示）**
     ```bash
     python manage.py bootstrap_demo
     ```
     该命令会生成：
      - 超级管理员：`admin / admin123`
      - 教师：`carol`、`dave`（如果尚无密码，自动设置为 `12345678` 并要求首次登录修改）
      - 学生：`alice`、`bob`（如果尚无密码，自动设置为 `12345678` 并要求首次登录修改）
      - 不少于 50 门课程、100 名学生的演示数据（含选课记录、通过/未通过成绩、培养方案和选课窗口），覆盖计算机与数学示例培养方案，可直接在 `/plans/` 查询

   - **小规模快速演示数据（约 10 名学生，便于快速加载或测试）**
     ```bash
     python manage.py bootstrap_mini
     ```
     该命令会生成管理员账号与约 10 名学生/少量课程的轻量级数据集，更适合本地快速试用或演示。
...
### 管理员常用操作
- **批量同步班级课表**：登录管理员工作台（`/accounts/home/admin/`），在“按班级同步课表”区域选择班级和多个教学班，提交后系统会为全班学生创建/更新对应选课记录。
- **按学院/班级批量分配课程**：同一页面“按学院/班级批量分配课程”区域，可按学院/班级/专业关键词一次性分配单个教学班。
...
- **Enrollment**：选课状态（选课中/退课/通过/未通过）、最终成绩及绩点，限制同一学生同一教学班唯一。
...
## 默认账号策略
- 在 Django Admin 中创建用户时，系统会自动分配默认密码 `12345678`，无须手动输入密码。
- 教师/学生新账号首次登录会被强制跳转到密码修改页面；管理员账号默认无需强制改密。

## 学生/教师自助与审批流
- 面向学生的自助前台：报名、退课、重修申请等请求可在线提交并查看进度。
- 重修、跨院选课、超学分等事项进入审批流，包含审核日志留痕，教师/管理员均可处理。
